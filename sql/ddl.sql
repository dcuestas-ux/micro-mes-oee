...
CREATE TABLE IF NOT EXISTS shift_calendar (
    ...
    name text NOT NULL,
    dow int[7] NOT NULL, -- 1..7 (Monday to Sunday)
...
CREATE TABLE IF NOT EXISTS event_state (
    ...
    state text NOT NULL CHECK (state IN ('IDLE', 'RUN', 'STOP', 'SETUP')),
    reason_code text,
    order_id text,
    lot_id text,
    seq bigint NOT NULL,
...
SELECT time_bucket('1 hour', time) AS bucket,
       ...
       sum(scrap) AS scrap_count,
       min(ct_ideal_ms) AS min_ct_ideal_ms,
FROM event_cycle
